...
if __name__ == '__main__':
    ...
    func2 = ODEFunc2().to(device)
    pp = 0
    for param in func2.net.named_parameters():
        param = params_orig[pp]
        pp+=1
    pred_y2 = odeint(func2, true_y0, t)
    visualize(true_y, pred_y2, func2, ii+1)
    func3 = ODEFunc().to(device)
    pp = 0
    for param in func3.net.named_parameters():
        param = params_orig[pp]
        pp+=1
    pred_y3 = odeint(func3, true_y0, t)
    visualize(true_y, pred_y3, func3, ii+2)
    nn1 = dict({'Wb':params,'names':names})
